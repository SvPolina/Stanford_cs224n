...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2, no_char_decoder=False):
        ...
        self.decoder = nn.LSTMCell(hidden_size+embed_size,self.hidden_size,bias=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X=self.model_embeddings_source(source_padded)       #shape (src_len, b, e)
        ...
        last_h=torch.cat((last_hidden[0],last_hidden[1]),1)        
        init_decoder_hidden=self.h_projection(last_h)
        
        last_c=torch.cat((last_cell[0],last_cell[1]),1)        
        init_decoder_cell=self.c_projection(last_c)  
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        outputs = self.model_embeddings_target(target_padded)

        for output in torch.split(outputs, split_size_or_sections=1):
            output = output.squeeze(0)
            output_t = torch.cat([output, o_prev], dim=-1)
            dec_state, o_t, _ = self.step(output_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        alpha_t_v = (alpha_t.size(0), 1, alpha_t.size(1))
        a_t = torch.bmm(alpha_t.view(*alpha_t_v), enc_hiddens).squeeze(1)
