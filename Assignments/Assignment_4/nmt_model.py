...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        last_hidden_conc   =torch.cat((last_hidden[0], last_hidden[1]), 1)        
        init_decoder_hidden=self.h_projection(last_hidden_conc)
        
        last_cell_conc     =torch.cat((last_cell[0], last_cell[1]), 1)        
        init_decoder_cell=self.h_projection(last_cell_conc)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Yt in torch.split(Y,1,dim=0):
            Yt_sq  = torch.squeeze(Yt)
            Ybar_t =torch.cat((Yt_sq, o_prev),dim=1) 
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ###
        new_shape = (alpha_t.size(0), 1, alpha_t.size(1))
        a_t = torch.bmm(alpha_t.view(*new_shape), enc_hiddens).squeeze(1)
