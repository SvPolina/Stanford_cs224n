...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for sent in sents:
            snt_ids=[]
            for word in sent:                              
                ids=[self.start_of_word]+[self.char2id[let] for let in word]+[self.end_of_word]
                snt_ids.append(ids)                
            word_ids.append(snt_ids)
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts        
        indc_mat=pad_sents_char(self.words2charindices(sents), self.char2id['<pad>'])
        output_tensor=torch.tensor(indc_mat).permute(1, 0, 2).to(device)
        ### END YOUR CODE
